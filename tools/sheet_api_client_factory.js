...
const SCOPES = ['http://www.googleapis.com/auth/spreadsheets'];
const TOKEN_PATH = 'accessToken.json';

class SheetApiClientFactory {
  static async create() {
    // 구글 OAuth 클라이언트 사용을 위해서 credentials.json 파일 읽기
    ...
    // 생성된 OAuth  클라이언트를 이용해서 구글 시트  API 클라이언트 생성
  ...
  static async _authorize(credentials) {
    ...
    // 기존에 발급받아둔 엑세스 토큰이 없다면 새롭게 발급 요청
